...
try:
    ctypes.winll.shcore.SetProcessDpiAwarness(1)
...
def draw_rectangle(x, y, width, height, color):
    ...
    screen.draw.line((X(x), y + height), (X(x + height), y + height), color)
...
def draw():
    ...
    screen.draw.line((X(0), horizon_y), (X(1000), horizon_y), MAGENTA)
    ...
    for x, y, w, h in left_buildings:
        ...
        screen.draw.line(
            (X(x), y), (X(x + offset), y - offset), MAGENTA
        )
        screen.draw.line(
            (X(x + w), y), (X(x + w + offset), y - offset), MAGENTA
        )

        screen.draw.line(
            (X(x), y + h), (X(x+offset), y +h - offset), MAGENTA
        )
        screen.draw.line(
            (X(x + w), y + h), (X(x + w + offset), y + h- offset), MAGENTA
        )
    ...
    for x, y, w, h in left_buildings:
        ...
        screen.draw.line(
            (X(x), y), (X(x + w - offset), y - offset), CYAN
        )
        screen.draw.line(
            (X(x + w), y), (X(x + w - offset), y - offset), CYAN
        )

        screen.draw.line(
            (X(x), y + h), (X(x-offset), y +h - offset), CYAN
        )
        screen.draw.line(
            (X(x + w), y + h), (X(x + w - offset), y + h- offset), CYAN
        )
